...
resource "aws_lb_target_group_attachment" "public-target-group" {
...
resource "aws_lb_target_group_attachment" "private-target-group" {
...
resource "aws_lb" "loadbalancer" {
    count                      = 2
    name                       = "sprints-loadbalancer-${count.index}"
